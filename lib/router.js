Router.configure({
  ...
  waitOn: function () {
    return [Meteor.subscribe('orders'), Meteor.subscribe('users')];
  }
});
...
Router.route('/userpage/:username', {
  name: 'userPage',
  data: function(){
...
Router.onBeforeAction(requireAdmin, {
  only: ['orderSubmit', 'orderEdit', 'accountCreate', 'usersList']
});

Router.onBeforeAction(requireLogin, {only: ['acceptedOrdersList']});
